...
class TransferError(RuntimeError):
    """Error domain khusus untuk kegagalan proses transfer SDR."""
...
@dataclass
class SDRConfig:
    """Konfigurasi radio, gain, buffer, dan parameter PHY yang dipakai transfer."""
...
class TransferMetadata:
    """Metadata file yang ikut dibawa selama sesi transfer."""
...
@dataclass
class Packet:
    """Representasi paket logis untuk hello, data, dan fin."""
...
@dataclass
class PacketDecode:
    """Hasil dekode paket beserta metrik kualitas link saat paket ditemukan."""
...
@dataclass
class TransferStats:
    """Statistik throughput dan durasi untuk satu sesi transfer selesai."""
...
def bytes_to_bits(data: bytes) -> np.ndarray:
    """Mengubah byte menjadi array bit uint8 agar mudah dimodulasi."""
...
def bits_to_bytes(bits: np.ndarray, byte_length: int | None = None) -> bytes:
    """Mengubah array bit kembali ke byte dengan padding seperlunya."""
...
class OFDMModem:
    def __init__(self, config: SDRConfig, payload_modulation: str = "qpsk") -> None:
        """Menyiapkan parameter OFDM, subcarrier, dan preamble pelatihan kanal."""
    ...
    def symbol_samples(self) -> int:
        """Jumlah sampel per simbol OFDM setelah ditambah cyclic prefix."""

        return self.fft_len + self.cp_len

    @property
    def preamble_samples(self) -> int:
        """Jumlah sampel yang dipakai oleh preamble sinkronisasi."""

        return self.preamble_baseband.size

    def _ifft_carriers(self, carriers: np.ndarray) -> np.ndarray:
        """Mengubah simbol-domain frekuensi menjadi simbol OFDM domain waktu."""
    ...
    def _qpsk_demap(self, symbols: np.ndarray) -> np.ndarray:
        """Mengambil keputusan keras QPSK pada komponen I dan Q."""
    ...
    def _modulate_bits(self, bits: np.ndarray, scheme: str, bits_per_symbol: int) -> tuple[np.ndarray, int]:
        """Membentuk deretan sampel OFDM dari stream bit payload atau header."""
    ...
    def _demodulate_symbols(
        ...
        """Mendekode sejumlah simbol OFDM menjadi bit memakai equalizer kanal."""
    ...
    def encode_packet(self, packet: Packet) -> np.ndarray:
        """Membungkus header dan payload menjadi satu waveform RF kompleks."""
    ...
    def packet_sample_length(self, payload_length: int) -> int:
        """Mengestimasi panjang waveform paket untuk payload tertentu."""
    ...
    def _decode_downmixed(self, downmixed: np.ndarray) -> PacketDecode:
        """Mendekode satu kandidat paket yang sudah diturunkan ke baseband."""
    ...
    def decode_packet_at(self, samples: np.ndarray, preamble_start: int) -> PacketDecode:
        """Mencoba mendekode paket di posisi kandidat dengan beberapa varian fase."""
    ...
    def find_candidates(self, samples: np.ndarray) -> list[int]:
        """Mencari puncak korelasi preamble yang mungkin menandai awal paket."""
...
class ReliableImageTransfer:
    def __init__(self, config: SDRConfig, modulation: str) -> None:
        """Menyatukan logika sesi transfer reliabel di atas modem OFDM."""
    ...
    def connect(self) -> None:
        """Membuka koneksi ke Pluto/AD936x dan menerapkan konfigurasi radio."""
    ...
    def close(self) -> None:
        """Menutup buffer TX/RX agar sesi berikutnya mulai dari kondisi bersih."""
    ...
    def _require_sdr(self) -> adi.ad9361:
        """Memastikan objek SDR sudah terhubung sebelum dipakai."""
    ...
    def _set_gains(self, tx_gain_db: int, rx_gain_db: int) -> None:
        """Mengubah gain TX/RX selama proses kalibrasi link."""
    ...
    def _start_cyclic_tx(self, waveform: np.ndarray) -> None:
        """Memulai transmisi cyclic agar receiver bisa menangkap waveform stabil."""
    ...
    def _capture_samples(self, capture_samples: int, warmup_buffers: int = 1) -> np.ndarray:
        """Mengambil sejumlah buffer RX lalu menggabungkannya menjadi satu array."""
    ...
    def _decode_samples(self, samples: np.ndarray, transfer_id: int | None = None) -> list[PacketDecode]:
        """Mencari dan mendekode semua paket valid yang ada di dalam sampel RX."""
    ...
    def _waveform_score(self, samples: np.ndarray, waveform: np.ndarray) -> tuple[float, int]:
        """Mengukur kekuatan kemiripan antara capture RX dan waveform referensi."""
    ...
    def auto_calibrate(self, hello_waveform: np.ndarray) -> tuple[int, int, float]:
        """Mencoba beberapa kombinasi gain lalu memilih link score terbaik."""
    ...
    def _transmit_window(self, packets: list[Packet], threshold_score: float) -> tuple[dict[int, bytes], int]:
        """Mengirim satu jendela paket dan memvalidasi hasil tangkapan receiver."""
        ...
        samples = self._capture_samples(capture_length, warmup_buffers=2)
        window_score, _ = self._waveform_score(samples, waveform)
        if window_score >= threshold_score:
            return {packet.sequence: packet.payload for packet in packets}, waveform.size
    ...
    def _build_metadata(self, input_path: Path, data: bytes, chunk_size: int, modulation: str) -> TransferMetadata:
        """Menyusun metadata file dan transfer_id unik untuk satu sesi kirim."""
    ...
    def transfer_file(
        ...
        """Menjalankan satu sesi transfer end-to-end sampai file diterima utuh."""
...
def main() -> int:
    """Entry point CLI dengan retry sesi untuk mengatasi startup SDR intermiten."""
